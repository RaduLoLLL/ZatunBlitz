...
const ReservationEditForm = ({ booking }) => {
  ...
  const CalculatePrice = () => {
    ...
    if (booking.total_price.toFixed(2) !== totalPrice.toFixed(2)) {
      return (
        <>
          <p className="my-6 font-bold">Pret initial: {booking.total_price} Lei</p>
          <p className="my-6 font-bold text-red-600 text-2xl">
            Pret final: {totalPrice.toFixed(2)} Lei
          </p>
        </>
      )
    }
  ...
  async function handleSubmit(event) {
    ...
    const booking: booking = {
      ...
      intrare_complex: Number(state.intrare),
      loc_parcare: Number(state.locParcare),
